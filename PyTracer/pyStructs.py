...
class Triangle(Primitive):
    ...
    def intersect(self, ray: Ray, info: RayIntersectionInfo):
        e0 = self.__v1.astype('float64') - self.__v0.astype('float64')
        e1 = self.__v0.astype('float64') - self.__v2.astype('float64')
        n = np.cross(e1, e0)
        e2 = (1.0 / np.dot(n, ray.direction.astype('float64'))) * (self.__v0.astype('float64') - ray.origin.astype('float64'))
        i = np.cross(ray.direction.astype('float64'), e2)
        beta = np.dot(i, e1)
        gamma = np.dot(i, e0)
        t = np.dot(n, e2)
    ...
    def all_intersect(self, ray: Ray, info: RayIntersectionInfo):
        ...
        e1 = self.__v0.astype('float64') - self.__v2.astype('float64')
        n = np.cross(e1, e0)
        e2 = np.true_divide(1.0, np.dot(n, ray.direction.astype('float64') )) * (self.__v0.astype('float64') - ray.origin.astype('float64'))
        i = np.cross(ray.direction.astype('float64'), e2)
        beta = np.dot(i, e1)
        gamma = np.dot(i, e0)
        t = np.dot(n, e2)
