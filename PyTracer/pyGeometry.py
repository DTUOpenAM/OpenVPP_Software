...
from PySide2.QtCore import Signal, Slot, QFileInfo, QObject, QTime
from external_libraries.rayTracerPyWrapper.rayTracerPyWrapper import PyBindBVH, PyBindRay, PyBindRayInfo
import sys
...
class PyGeometry(QObject):
    ...
    def __init__(self, filename='', vertices=[], normals=[], bbox_min=QVector3D(), bbox_max=QVector3D(), use_bvh=False):
        ...
        if use_bvh and len(self.vertices_list) > 0:
            ...
            t = time.elapsed()
            if CPP_SLICER:
                self.c_bvh = PyBindBVH(self.vertices_list.tolist(), PyBindBVH.EqualCounts, 255)
                print("BVH time:", time.elapsed() - t)
            else:
