...
def bvh_cube_test():
    ...
    v6 = np.array([-l, l, l])
    v7 = np.array([-l, l, -l])
    primitives.append(st.Triangle(v0, v2, v1))
    primitives.append(st.Triangle(v3, v1, v2))
    primitives.append(st.Triangle(v1, v0, v5))
    primitives.append(st.Triangle(v5, v0, v4))
    primitives.append(st.Triangle(v1, v5, v3))
    primitives.append(st.Triangle(v3, v5, v7))
    primitives.append(st.Triangle(v5, v6, v4))
    primitives.append(st.Triangle(v6, v5, v7))
    primitives.append(st.Triangle(v6, v7, v2))
    primitives.append(st.Triangle(v2, v7, v3))
    primitives.append(st.Triangle(v2, v0, v6))
    primitives.append(st.Triangle(v0, v4, v6))
    start_time = time.time()
    bvh = pyBVH.BVH(primitives, 'EqualCounts')
    print("construction time", time.time() - start_time)
    start_time = time.time()
    for i in range(number_of_rays):
        ray_info = st.RayIntersectionInfo()
        ray_dir = np.array([0, 0, -1])
        ray_o = np.array([-l + i * offset, 0, l+1])
        ray = st.Ray(ray_o, ray_dir, 0, 1000, 0, 0)
        bvh.intersect(ray, ray_info)
        # print(ray_info.t_hits)
    print("intersection time", time.time() - start_time)
    start_time = time.time()
    plane = st.Plane(np.array([0,0,0]), np.array([0,1,0]))
    pinfo = st.PlaneIntersectionInfo()
    bvh.plane_all_intersections(plane, pinfo)
    print("plane intersection time", time.time() - start_time)
