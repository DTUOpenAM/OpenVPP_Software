...
class MetalSlicer(QOpenGLWidget, QOpenGLFunctions):
    ...
    def __init__(self, parent=None):
        ...
        self.contour_strategies = ['Geometric', 'None']
    ...
    def __append_slicing_default_parameters__(self):
        ...
        self.contour_vertices_list.append({'vertices': np.array([], dtype=np.float32), 'vertices_per_layer': [], 'layer_thickness': 0})
        self.infill_vertices_list.append({'vertices':  np.array([], dtype=np.float32), 'vertices_per_layer': [], 'layer_thickness': 0})
    ...
    def prepare_for_slicing(self, directory='./'):
        ...
        for idx in range(self.geometries_loaded):
            ...
            self.contour_vertices_list[idx] = {'vertices': np.array([], dtype=np.float32), 'vertices_per_layer': [], 'layer_thickness': self.slice_thickness_microns / 1000.0}
            self.infill_vertices_list[idx] = {'vertices':  np.array([], dtype=np.float32), 'vertices_per_layer': [], 'layer_thickness': self.slice_thickness_microns / 1000.0}
    ...
    def interrupt_slicing(self):
        if self.is_slicing:
            ...
            self.is_slicing = False
            for idx in range(self.geometries_loaded):
                self.contour_vertices_list[idx] = {'vertices': np.array([], dtype=np.float32), 'vertices_per_layer': [], 'layer_thickness': 0}
                self.infill_vertices_list[idx] = {'vertices':  np.array([], dtype=np.float32), 'vertices_per_layer': [], 'layer_thickness': 0}
    ...
    def slice_next_layer(self):
        if self.geometries_loaded > 0:
            ...
            for geometry_idx in range(self.geometries_loaded):
                        ...
                        # print("contour total time", time.elapsed() - t)
                        # t = time.elapsed()
                        self.__get_slice_infill(geometry_idx)
                        # print("infill total time", time.elapsed() - t)
            # print("slice", self.current_slice, "total time", time.elapsed() - t0)
        ...
        if self.current_slice == self.number_of_slices:
            ...
            print("loading buffers", time.elapsed() - t)
            t = time.elapsed()
            self.__reset_default_opengl_buffer__()
            self.is_slicing = False
            self.update_slice_counts.emit(self.current_slice, self.number_of_slices)
            print("finalizing slicing", time.elapsed() - t)
            t = time.elapsed()
    ...
    def __get_slice_contour(self, geometry_idx):
        ...
        if self.contour_strategies[strategy_idx] == 'Geometric':
            ...
            self.contour_vertices_list[geometry_idx]['vertices'] = np.concatenate(
                    (self.contour_vertices_list[geometry_idx]['vertices'], np.array([], dtype=np.float32)), axis=None)
    ...
    def __get_slice_infill(self, geometry_idx):
        ...
        if self.infill_strategies[strategy_idx] == 'Parallel Lines':
            ...
            self.infill_vertices_list[geometry_idx]['vertices'] = np.concatenate(
                    (self.infill_vertices_list[geometry_idx]['vertices'], np.array([], dtype=np.float32)), axis=None)
    ...
    def __compute_slice_plane_contour(self, geometry_idx):
        ...
        if not pyGeometry.CPP_SLICER:
            ...
            hits = slice_plane_info.intersections
        else:
            slice_plane = PyBindPlane(plane_x0, plane_normal)
            slice_plane_info = PyBindPlaneInfo()
            hits = current_geometry.get_c_bvh().PlaneAllIntersectsHits(slice_plane, slice_plane_info)
        if len(hits) > 0:
            slice_contour = (np.array(hits, dtype=np.float32).dot(transformation_matrix[:3, :3].transpose())
                             + np.array(transformation_matrix[0:3, 3])).ravel()
            rounded = np.round(slice_contour, self.__number_of_decimals)
            self.contour_vertices_list[geometry_idx]['vertices'] = np.concatenate((self.contour_vertices_list[geometry_idx]['vertices'], rounded), axis=None)
    ...
    def __get_parallel_lines_infill(self, geometry_idx):
        ...
        if number_of_rays == 0:
            ...
            self.infill_vertices_list[geometry_idx]['vertices'] = np.concatenate(
                    (self.infill_vertices_list[geometry_idx]['vertices'], intersection_points), axis=None)
        ...
        if not pyGeometry.CPP_SLICER:
            rays_origins = [inverse_matrix.dot(rotation_matrix.dot(np.array([ray_origin_x + rays_origin_offset * idx, ray_origin_y, ray_origin_z, 1.0], dtype=np.float32))
                            + np.array([bbox_center.x(), 0, bbox_center.z(), 0], dtype=np.float32))[0:3]for idx in range(number_of_rays)]
            ...
            ray_origin = np.array([ray_origin_x, ray_origin_y, ray_origin_z])
            b_center = np.array([bbox_center.x(), bbox_center.y(), bbox_center.z()])
            intersection_points = current_geometry.get_c_bvh().MultiRayAllIntersectsTransformHits(ray_origin, ray_direction, number_of_rays, b_center, rays_origin_offset, rotation_matrix, inverse_matrix)
        try:
            ...
            intersection_points = np.array([], dtype=np.float32)
        rounded = np.round(intersection_points, self.__number_of_decimals)
        self.infill_vertices_list[geometry_idx]['vertices'] = np.concatenate(
                (self.infill_vertices_list[geometry_idx]['vertices'], rounded), axis=None)
        self.infill_vertices_list[geometry_idx]['vertices_per_layer'].append(len(intersection_points))
    ...
    def __load_slices_buffers__(self):
        ...
        # time.start()
        for geometry_idx in range(self.geometries_loaded):
            rounded_vertices = self.contour_vertices_list[geometry_idx]['vertices']
            GL.glBindBuffer(GL.GL_ARRAY_BUFFER, self.contour_buffer_id_list[geometry_idx])
            GL.glBufferData(GL.GL_ARRAY_BUFFER, rounded_vertices.nbytes, rounded_vertices, GL.GL_STATIC_DRAW)
            rounded_vertices = self.infill_vertices_list[geometry_idx]['vertices']
            GL.glBindBuffer(GL.GL_ARRAY_BUFFER, self.infill_buffer_id_list[geometry_idx])
            GL.glBufferData(GL.GL_ARRAY_BUFFER, rounded_vertices.nbytes, rounded_vertices, GL.GL_STATIC_DRAW)
    ...
    def write_slice_job_to_file(self, file_name="./job.g"):
        ...
        if file.open(QIODevice.WriteOnly | QIODevice.Text):
            ...
            for slice_idx in range(self.number_of_slices):
                ...
                stream << "M10\n"
                stream << "C2 A0\n"
                stream << "M4 V1 T7500 D683 F490\n"
                stream << "M4 V2 T7500 D683 F490\n"
                stream << "M4 V3 T7500 D683 F490\n"
                stream << "M7 R30\n"
