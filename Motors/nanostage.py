...
class NanoStage(QObject):
    ...
    @Slot()
    def connect_motor(self, serial_port, trq_limit=60):
        if serial_port == '':
            self.print_text_signal.emit("A valid port was NOT selected! Select a valid port.")
            return False
        try:
            # serial_number = int(serial_port[-1])
            # self.gcs.ConnectRS232(comport=serial_number, baudrate=115200)
            self.gcs.ConnectUSB(serialnum='0019550184')
